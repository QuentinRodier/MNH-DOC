...
\begin{tabular}{>{\centering}p{3cm}>{\centering}p{2.5cm}|p{11cm}|}
\cline{3-3}
&&{\tt UM10, VM10}    : [2D] zonal and meridional wind at 10\,m height AGL (m/s)\\\cline{3-3}
&&{\tt FF10MAX}       : [2D] wind gusts at 10\,m height AGL, with TKE component equals to $4\sqrt{\mathrm{TKE}_\mathrm{IKB}}$ (m/s, only if CTURB='TKEL')\\ \cline{3-3}
&&{\tt FF10MAX2}      : [2D] wind gusts at 10\,m height AGL, with TKE component equals to $4\sqrt{\mathrm{TKE}_\mathrm{10m}}$ (m/s, only if CTURB='TKEL')\\ \cline{3-3}
&&{\tt FF10MAX\_AROME} : [2D] wind gusts at 10\,m height AGL, with TKE component equals to $3.8\sqrt{\mathrm{TKE}_\mathrm{20m}}$ as in AROME-France (m/s, only if CTURB='TKEL')\\ \cline{3-3}
...
Since RTTOV requires a license agreement, no RTTOV package is included in the open source version of Meso-NH. However, a subroutine calling RTTOV 13 is included in Meso-NH version Masdev5\_7. {\bf To use RTTOV, you must follow the instructions in the A-INSTALL file to compile MESO-NH with RTTOV.}   
...
\end{verbatim}
For this specific choice, you need to have four files:
