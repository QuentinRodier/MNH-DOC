...
\item
CELEC = 'ELE3'  : original scheme CELLS\footnote{see Barthe and Pinty, 2007; Barthe et al. 2007, 2012, 2016)} based on duplication of microphysics ICE3 code. Works only with NAM\_PARAM\_ICEnn LRED=F, LSNOW\_T=F.
\item
...
With ICE3, set LRED=T and LSNOW\_T=F in NAM\_PARAM\_ICEnn.
\end{itemize}
...
\item 
NMAXITER  \index{NMAXITER!\innam{NAM\_PARAM\_ICEn}}: Maximum number of iterations for mixing ratio or time splitting (active when LRED=T)
...
\item
LPACK\_MICRO  \index{LPACK\_MICRO!\innam{NAM\_PARAM\_ICEn}}: the microphysics input variables are packed to perform the computation only on points with hydrometeors; otherwise, computations are performed everywhere. Do not change this option with Méso-NH : .FALSE. works only for AROME).

\item 
CFRAC\_ICE\_ADJUST   \index{CFRAC\_ICE\_ADJUST!\innam{NAM\_PARAM\_ICEn}}: Way to compute ice fraction 
\begin{itemize}
...
\item 
CFRAC\_ICE\_SHALLOW\_MF \index{CFRAC\_ICE\_SHALLOW\_MF!\innam{NAM\_PARAM\_ICEn}}: Way to compute ice fraction for MF contribution
\begin{itemize}
\item{'T'} : linear formulation according to temperature
\item{'O'} : Tao et al. (1989) formulation
\item{'N'} : No ice
