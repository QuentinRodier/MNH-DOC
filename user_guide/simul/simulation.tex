...
NBAK\_TIME\_FREQ\_FIRST(m): timestep number of the first backup for each model $m$ (if NBAK\_STEP\_FREQ(m) is set)
...
NCMELRC  & collection by snow and conversion into rain with T>XTT & integer  &  0 \index{NCMELRC!\innam{NAM\_BU\_RRC}}\\
(if CCLOUD='ICEx' .AND. LRED) &on ice &   &  \\\hline
...
(if CCLOUD='ICEx' .AND. LRED) &on ice &   &  \\\hline
NADJURC  & adjustement before  & integer  &  0 \index{NADJURC!\innam{NAM\_BU\_RRC}}\\
(if CCLOUD='ICEx' .AND. LRED) &on ice &   &  \\\hline
...
(if CCLOUD='ICE4' and LRED) &  on ice&   &  \\\hline
...
NHGCVRG  & Hail to Graupel ConVersion & integer  &  0 \index{NHGCVRG!\innam{NAM\_BU\_RRG}}\\
(if CCLOUD='ICE4' and LRED) &   &   &  \\\hline
NGHCVRG  & Graupel to Hail ConVersion & integer  &  0 \index{NGHCVRG!\innam{NAM\_BU\_RRG}}\\
(if CCLOUD='ICE4' and LRED) &   &   &  \\\hline
...
(if CCLOUD='ICE4') & &   &  \\\hline
NCMELRR  & collection of droplets by snow and conversion into rain & integer  &  0 \index{NCMELRR!\innam{NAM\_BU\_RRR}}\\
(if CCLOUD='ICEx' and LRED) & &   &  \\\hline
...
\begin{itemize}
\item 0 : no messages
\item 1 : debug messages (and higher levels)
\item 2 : info messages (and higher levels)
\item 3 : warning messages (and higher levels)
\item 4 : error messages (and higher levels)
\item 5 : fatal messages (and higher levels)
\end{itemize}
...
\item
COUT\_VAR(m,p): list of the field names to output for each model $m$.
...
Remarks:
\begin{itemize}
\item Not all fieldnames are possible. If a field is not (yet) knwon, it is possible to add a personalized one by modifying the $IO_WRITE_FIELD_USER$ subroutine.
\item If a choosen time is not a multiple of the timestep, it will be rounded to the nearest one.
