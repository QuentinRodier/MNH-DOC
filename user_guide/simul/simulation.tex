...
CCCN\_MODES : Select the size distribution of CCN modes \\ ('JUNGFRAU','COPT','CAMS','CAMS\_JPP','CAMS\_ACC','CAMS\_AIT','SIRTA','CPS00','FREETROP')
\item
...
\item
CTURBLEN='ADAP'
The mixing length is computed according to Honnert et al. (2021). For large horizontal grid meshes, this is equal to RM17. For LES grid meshes, this is equivalent to DEAR : the base mixing lentgh is the horizontal grid mesh, and it is limited by a stability-based length (RM17), as was done in DEAR (but taking into account shear as well). For grid meshes in the grey zone, ADAP is the smaller of the two.
\end{itemize}
